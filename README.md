...
一套面向 Hermes Agent 当前源码的中文深度学习笔记。

这里不做源码目录翻译，也不把 Agent 简化成“模型加几个工具”。课程沿着真实运行链讲解 Prompt、工具调用、会话状态、上下文压缩、长期记忆、安全控制、多 Agent 协作和产品入口，并在相同问题上对比 Codex 与 Claude Code。
...
每篇笔记先提出一个具体工程问题，再找到 Hermes 的源码入口。源码只摘取理解控制流所需的短片段，后面紧跟状态变化、设计原因和失败路径。
...
源码锚点全部使用 Hermes 仓库相对路径。读者可以在对应源码快照中继续追踪定义和调用方，不需要依赖本仓库作者的本机目录。
...
第一遍先跟着每篇的功能主线走，不急着记函数名。读完一个功能后，先尝试回答“输入、状态、输出、失败路径分别是什么”。

第二遍再按源码锚点回到 Hermes 仓库。重点追踪定义、调用方和持久化位置。一个状态如果只在函数局部变量里，它通常只影响当前模型迭代；如果进入 Agent 实例、SessionDB 或 Memory，它的生命周期就完全不同。

遇到不理解的地方，可以围绕具体状态转换提问。例如：“tool call 已经写入 SessionDB，但工具还没有执行，此时进程退出会怎样？”这类问题比单问一个术语更容易触及设计本身，也适合整理成后续读者能检索的问答。
