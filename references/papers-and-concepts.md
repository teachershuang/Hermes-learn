...
这些说法会混淆工程边界。论文可能研究训练方法，API 文档描述协议，框架源码实现运行时，Docker 和操作系统负责隔离。它们经常出现在同一个 Agent 中，但不在同一层。
...
每个概念都需要回到同一个检查点：Hermes 的哪段代码在维护这个不变量？找不到源码或公开行为证据时，只能将其视为类比，不能写成已实现事实。
...
Hermes 没有发明上述研究概念，它的工程价值在于把模型调用、工具协议、会话检索、长期记忆、压缩、Skills、安全和委派接进同一个运行系统。论文解释机制为何可能有效，源码则给出输入不合法、网络中断、历史过长、并发冲突或权限不足时的实际处理方式。
