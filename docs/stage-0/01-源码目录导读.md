...
从目录树第一行顺序阅读，会把大量时间消耗在产品外壳、兼容代码和发布脚本上。本文按运行链划分目录，再根据调用关系确定阅读顺序。
...
它不是 `src/hermes_agent/...` 这种单包布局，生产代码也没有集中在一个同名 package 中。
...
代价也很明显。使用 `rg` 搜索方法名时，通常先定位到 façade，再跳到实际实现；`AIAgent` 仍拥有大量动态字段，类型边界没有完全收紧。
...
工具结果被追加为消息并重新进入模型调用后，一次工具闭环已经形成。完整时序见第 20 讲，本文只处理文件定位关系。
...
在 `cli.py` 定位到 `self.agent.run_conversation(...)` 时，上游仍是 UI 事件，下游才进入 Agent Runtime。CLI 的状态栏和键盘处理不属于 Agent Loop。
...
核对标准：command、profile、Provider、toolsets 和 session 的确定位置能够逐项定位。
...
核对标准：能够指出谁决定继续调用模型、谁决定结束，以及工具结果在哪一步回到 messages。
...
核对标准：能够区分“没有暴露”“调用被阻断”“handler 报错”和“后端环境失败”。
...
核对标准：能够区分稳定 system prompt、当前消息、冻结 Memory、压缩摘要和按需 `session_search`。
...
核对标准：能够说明平台身份怎样映射到 session，以及 Agent 输出怎样回到原聊天。
...
核对标准：能够区分客户端状态、宿主协议和 Agent Runtime 状态，避免把 UI 问题误判成模型问题。
...
`run_agent.py` 适合作为定位地图，不适合从第一行顺序阅读。入口是 `AIAgent` 的构造函数和 `run_conversation` 转发，随后应进入 `agent/agent_init.py` 与 `agent/conversation_loop.py`。当前 `run_agent.py` 同时承担公共 API、兼容逻辑和部分尚未迁出的实现。
...
定位功能时，应先确定它位于图中的哪个节点，再沿箭头追踪一条闭环。目录名称不能代替实际调用关系。

这套源码树记录了一个持续扩展的 Python Agent 产品怎样在保留兼容性的同时，逐步拆分 Runtime、工具、控制面、长期服务和多种客户端。函数调用需要放回这些实际边界中理解，不能只按目录套用整洁架构模板。
