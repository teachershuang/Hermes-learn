...
前面的课程已经分别讲过 Agent loop、Prompt Builder、Tool Registry、工具执行、SessionDB、上下文压缩和插件系统。这一讲不再拆零件，而是追踪一次真实对话。
...
以前的课程逐项讲过这条管线。本讲只说明它在完整调用中的先后关系。
...
这是一个需要记住的观测边界：调试“模型原本说了什么”应看 messages/SessionDB，调试“用户最终看到了什么”应看 final result 和平台投递日志。
...
Hermes 的 Agent loop 可以浓缩成一句话：宿主交入一条用户 turn，Runtime 反复把当前历史交给模型，把经过校验的动作交给环境，再把环境结果交回模型，直到得到可接受文本或明确的停止原因。

真正的工程工作都在“经过校验”“当前历史”和“明确停止”这几个词里。Transport 让多 Provider 共用循环，工具管线守住副作用，SessionDB 让崩溃后仍能解释发生过什么，finalizer 则保证不同退出路径最后回到同一种宿主结果。
