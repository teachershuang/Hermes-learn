...
Hermes 的无人值守任务不只是 `cronjob` 工具对 prompt 做延迟执行。源码将自动化拆成任务记录、时间计算、触发器、执行体、平台投递和安全边界，使 Agent 可以在用户离线后按计划创建独立运行并保存结果。
...
默认情况下，Hermes 进程中运行一个 60 秒 ticker；托管场景可以使用 Chronos，由外部服务到点后回调 Hermes。两种触发方式最终都进入同一个 `run_one_job`。
...
Provider 的 `fire_due` 只负责 claim 并调用共享执行体，不会重新实现 Agent 或投递逻辑。
...
cron job 的 final response 不由模型调用 `send_message` 发送。Hermes 在 prompt 中要求模型只输出最终内容，系统负责自动投递，从而分离内容生成与投递目标。
...
Cron job 是一个可恢复的未来 session，不是当前 session 的延迟消息。
