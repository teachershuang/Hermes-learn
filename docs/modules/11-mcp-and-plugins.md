...
`handle_401()` 的语义值得注意。它不承诺“401 后一定成功登录”，而是先检查磁盘是否已有外部刷新，再判断是否有 refresh token 可用，最后让下一个请求走 SDK 的刷新路径。这样避免每一次工具调用都无条件做一次网络 refresh。
...
你应能从 `mcp_servers` 的一段配置，解释它如何连接 server、发现工具、注册到 `mcp-<server>` toolset、被模型选中、执行并把不可信结果回填上下文。

你也应能解释为什么 Hermes 同时提供 MCP 和 Python plugin：前者接外部服务，后者改宿主行为；两者的信任模型、失败处理和安全责任不同。
