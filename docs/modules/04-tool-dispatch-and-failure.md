...
上一课讲的是工具怎么注册、怎么被模型看见。这一课讲模型真的发起 tool call 以后，Hermes 怎么把它变成真实工具执行。
...
上一课讲过 Tool Search：工具很多时，Hermes 可能把一部分 MCP / plugin 工具延迟披露，模型先调用 `tool_search`，再通过 `tool_call` 桥接调用底层工具。
...
这一课只覆盖模型主动调用工具时的 `memory` / `skill_manage`。如果模型在 tool call 分支里调用 `memory`，它就是普通工具调用的一部分。

后台的 memory review / skill review 不是同一个时机。它更接近 turn 结束后的整理逻辑，用来判断是否要沉淀长期记忆或技能。这个会在 Memory 和 Skills 章节继续讲。
