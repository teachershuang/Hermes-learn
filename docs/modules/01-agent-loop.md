...
本章分析用户消息进入 Hermes 后，主循环怎样推进以及各类退出路径怎样收尾。

Hermes 的一轮对话不是一次模型调用，而是一段受预算和中断条件约束的状态推进：
...
Hermes 因此在主循环前后都执行消息修复和清洗，并在 API 调用前再次 repair message sequence。
...
Hermes 还要服务 Gateway 和后台任务。用户可能在 Telegram 上发一条新消息打断旧任务，也可能有 cron 或子 Agent 正在运行。因此中断和预算同时影响 CLI 体验与跨平台会话一致性。
...
Hermes 会尽量把可恢复错误反馈给模型，而不是立刻终止。截断、预算耗尽和安全拦截等情况则更倾向结束当前路径。
...
### Memory / Skill review 在什么时机触发？

`memory` 工具可以在 tool call 分支中由模型主动调用。memory review 与 skill review 则在 turn 收尾后按计数条件启动后台任务，用于判断是否沉淀长期记忆或技能。两条路径不共享触发时机，完整实现分别见 Memory 和 Skills 章节。
