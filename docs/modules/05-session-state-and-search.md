...
前面几讲已经把主循环、prompt builder、tool registry 和 tool call 分支串起来了。现在要补上一个更容易被忽略的部分：一轮对话结束后，这些消息到底去了哪里？下次用户说“继续”，或者模型需要查找之前的讨论时，Hermes 怎么把旧上下文拿回来？
...
Hermes 没有把会话历史简单写成一堆 JSONL 文件，而是在 `hermes_state.py` 里实现了一个 SQLite 会话库。这个选择很工程化：SQLite 足够轻，不需要单独部署服务；FTS5 能做全文检索；WAL 模式适合“多个读者、一个写者”的桌面和 gateway 场景。
...
`parent_session_id` 很关键。它不是单纯的“树形历史好看一点”，而是为了处理压缩后的续接、分支会话、子代理会话。后面讲 `resolve_resume_session_id` 时会看到，Hermes 不能随便沿着父子关系往下走，否则恢复会话可能被带到一个不相干的 subagent 会话里。
...
第二，tool call 和 tool result 都会落库。上一讲说过，assistant 带 `tool_calls` 的消息必须和后续 `role="tool"` 的结果配对。SessionDB 保存这对关系，恢复会话时才能重新构造 provider 接受的消息序列。
...
你之前问过：在 Codex 里经常说“继续”，Hermes 里这个上下文管理在哪里？
...
第一，FTS 表只索引一个 `content` 字段，但它拼进了 `content`、`tool_name`、`tool_calls`。这意味着搜索不只覆盖用户和 assistant 文本，也能搜到工具名和工具调用参数。比如你过去让 agent 用某个工具改过文件，检索时有机会通过工具调用内容找到那段历史。
...
这是为中文、日文、泰文等脚本准备的。默认 `unicode61` 分词器对 CJK 不友好，短语搜索容易被拆坏。trigram 把文本切成重叠片段，更适合做中文子串检索。这个细节对中文用户很实际，因为我们检索旧会话时经常用中文关键词。
...
它会排除 `active=0, compacted=0` 的消息。这通常表示用户撤回、rewind 或不应再被使用的历史。换句话说，Hermes 区分了“被总结掉”和“被拿掉”。前者还能搜，后者默认不搜。
...
这意味着，如果用户给了一个文件、URL、GitHub issue、应用线程，agent 应该优先检查原始来源，而不是只搜历史。历史只能说明“我们以前怎么谈过”，不能证明“外部对象现在是什么状态”。

这个边界非常重要。很多 agent 出错，不是因为不会检索，而是把旧对话当成现实世界的证据。

## 功能 7：SessionDB、Memory、Prompt Builder 的边界

把这一讲和第二讲 Prompt Builder 连起来看，会更清楚。
...
### 3. session_search 算不算 memory？

不算。`session_search` 是检索过去对话的工具，返回的是历史消息。Memory 是从对话中提炼出来、准备长期复用的事实或偏好。

一个例子：你曾经在某次会话里说“我喜欢用中文写学习笔记”。这句话原文在 SessionDB；如果 Hermes 把它提炼成长期偏好，那才进入 Memory。以后模型可以通过 memory 直接知道这个偏好，也可以通过 `session_search` 找到你当时说这句话的上下文。

### 4. 为什么不把所有历史都塞进 prompt？
...
到这里，你应该能回答一个核心问题：Hermes 的上下文管理不只在 prompt builder，也不只在 agent loop。真正让它能“继续”的，是内存消息流、SessionDB、压缩机制和 session_search 共同形成的一条状态链。
