...
本章分析 `run_conversation` 发起每次 API 调用前，Hermes 怎样组装模型实际接收的系统提示、消息和工具定义。
...
### Skills 再讲深一点：它到底在 prompt 里做了什么

如果只把 Skills 理解成“工具列表”，这一段会很难读。Hermes 的 Skills 更接近一种程序化记忆：它记录一类任务应该怎样做、有哪些坑、要用哪些命令、哪些流程已经被验证过。

它和普通工具不是一层东西：

| 概念 | 解决的问题 | 典型形态 |
| --- | --- | --- |
| Tool | 现在要执行一个动作 | `read_file`、`execute_code`、`session_search` |
| Skill | 现在这类任务应该怎么做 | `SKILL.md` 里的步骤、约束、脚本、参考资料 |
| Memory | 长期事实和偏好 | 用户偏好、环境习惯、稳定约定 |

所以 `skill_view` 本身是一个工具，但它读取的是 Skill。模型不是“调用 Skill 去完成任务”，而是先用 `skill_view` 把相关 Skill 的说明读进上下文，然后按里面的流程继续调用普通工具。

流程可以这样看：

```mermaid
flowchart TD
    A["启动或恢复 session"] --> B["构建 system prompt"]
    B --> C["扫描 skills 目录"]
    C --> D["生成 compact skill index"]
    D --> E["模型看到技能名称和简介"]
    E --> F{"当前任务匹配某个 skill 吗？"}
    F -- "是" --> G["调用 skill_view(name)"]
    G --> H["读取完整 SKILL.md 和必要资源"]
    H --> I["按 skill 指令继续使用工具"]
    F -- "否" --> J["继续普通 agent loop"]
```

`build_skills_system_prompt` 做的是 C 和 D，不做 G。也就是说，它只把“技能目录”放进 system prompt，不把完整技能正文放进去。

这背后有三个工程原因。

第一，Skills 数量可能很多。每个 Skill 还可能引用 `references/`、`templates/`、`scripts/`、`assets/`。如果全塞进 system prompt，prompt 会非常大，而且大部分内容和当前任务无关。

第二，Skill 是否应该显眼，取决于当前工具集、平台和工作姿态。源码里 cache key 会考虑 available tools、toolsets、platform、disabled skills、compact categories。也就是说，同一个 Hermes，在 CLI、Gateway、某个受限平台里看到的 Skills index 可能不同。

第三，Hermes 先让模型知道某项能力存在，再按需加载细节。这与搜索引擎索引类似：索引列出可用内容，完整正文在实际打开后才进入上下文。

源码里这段 system prompt 很直接：
...
这句话不是装饰。它在改变模型的默认行为：不要只靠通用能力硬做任务，而是先检查有没有专门流程。

### 为什么 Skills 要做两层缓存

Skills index 每次构建都要扫描目录、解析 frontmatter、过滤平台、合并外部目录、处理分类压缩。如果每个 session 都全量扫一遍，启动成本会变高。
...
磁盘 snapshot
    进程重启后仍然可以复用
    通过 mtime/size manifest 判断是否过期
```

这不是单纯优化速度。更重要的是，Skills index 属于 system prompt 的 stable 层。如果它频繁变化，system prompt 前缀就不稳定，prefix cache 命中率也会下降。
...
Memory 与 Skills 章节分别说明两类信息的写入、加载和生命周期边界。
...
### Hermes 有没有类似 `CLAUDE.md` / `AGENTS.md` 的设计？

有，而且更宽。Hermes 会优先加载 `.hermes.md / HERMES.md`，然后才是 `AGENTS.md / agents.md`、`CLAUDE.md / claude.md`、`.cursorrules`。这些文件属于项目上下文文件，会进入 system prompt 的 context 层。

这里要注意：`system_message` 不是这些文件。`system_message` 是调用方通过 `run_conversation(..., system_message=...)` 传进来的自定义系统消息；项目上下文文件是 Hermes 自己从工作目录发现的文件。二者最后都进入 context 层，但来源不同。

### `_restore_or_build_system_prompt` 只是在判断模型和 provider 吗？

它主要做两件事：恢复同一个 session 已经存过的 system prompt，或者在不能恢复时重新构建。恢复时确实不会做语义比较，它只检查存储的 prompt 是否和当前 runtime 身份匹配，重点是 model/provider 这类运行时身份。

这是一种工程取舍。Hermes 要的是“原样复用同一个 prompt 前缀”，而不是“重新推理这个 prompt 语义上是否等价”。语义等价很难可靠判断，也会破坏 prefix cache 的确定性。

### USER profile 从哪里来？

USER profile 来自 Hermes 内置 memory store 里的 `user` 类数据。源码里是 `agent._memory_store.format_for_system_prompt("user")`。它不是项目里的 `USER.md` 文件，也不是当前用户消息，而是 Hermes 从长期记忆中整理出的用户画像，例如稳定偏好、工作习惯、表达方式等。

### 外部 memory block 是什么？

Hermes 有内置 memory store，也支持外部 memory provider。外部 provider 由 `agent._memory_manager` 管理，可以来自插件或第三方记忆系统。它们可以给 system prompt 增加一段说明，也可以在每轮开始前根据当前用户消息做 prefetch，把检索到的外部记忆作为临时上下文附加到当前 user message。

所以外部 memory 有两种进入模型的方式：长期规则说明进入 system prompt 的 volatile 层；本轮检索结果进入当前 user message。
