...
本章分析 Hermes 的系统边界：终端、浏览器、桌面应用和消息平台的输入分别经过哪些进程与协议进入 `AIAgent`。

重点不在界面操作，而在多种交互表面怎样复用同一 Agent 内核，以及界面状态、实时会话、持久化历史和平台身份怎样保持边界。
...
多个交互表面使用同一个 `SessionDB`，只表示它们可以读取同一份持久化历史，不表示它们共享同一个正在运行的 Python 对象。实时 session registry、当前 turn、流式 token 和待审批请求通常属于某个具体后端进程。
...
同一套 `server.dispatch` 也会被 `tui_gateway/ws.py` 暴露到 WebSocket。stdio 和 WebSocket 是两种 transport，RPC 方法与 Agent 行为仍由同一个 server 模块决定。
...
这些关口的顺序决定实际行为。审批回复必须早于 busy policy，因为它属于当前 turn 的控制输入；鉴权必须早于 Agent 构建，因为未授权消息不应触发昂贵初始化；ContextVar 必须最先重置，因为异步任务会继承创建时的上下文。
...
它适合“不必打断当前工作，但这件事随后要做”。源码除字符串外还会保留 `MessageEvent` 边界，因为媒体、回复关系和平台 metadata 也属于输入。显式 `/queue` 的多条消息还需要 overflow 列表，不能让后来的消息覆盖前一条。
...
UI Gateway 能提交 prompt、调用工具、读取 session 和修改配置，属于 Agent 控制面。只监听 loopback 时，Dashboard 可以使用临时 session token。绑定非 loopback 地址时则必须启用更严格的认证，WebSocket 还要单独检查 host、origin 和 ticket/token。
...
“Codex 是 CLI，Hermes 是 Gateway”无法描述两者的系统边界。两边都在解耦 Agent Runtime 与客户端；Hermes 还在同一仓库中实现了面向消息平台的常驻运行时。
...
Hermes 在不同入口之间复用 Agent 内核和持久化能力，但不会把所有入口放进同一个进程或同一种实时会话模型。
