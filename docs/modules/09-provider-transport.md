...
这一讲的目标是把这条链路讲清楚。看完以后，你应该能分清 `provider`、`model`、`base_url`、`api_mode`、`transport`、`profile`、`credential pool` 分别负责什么。
...
这套设计对长期维护很关键。模型供应商变化很快，如果每个 provider 都要改 `run_agent.py`，主循环迟早变成供应商兼容层。Hermes 把这部分搬到 plugin/profile 层，主循环只关心已经解析好的 runtime 信息。
...
这些仍然在 `AIAgent` 和相关 runtime helper 里。这个边界很重要。Transport 是数据格式转换层，不是“供应商客户端管理器”。
...
这也是为什么你看源码时会觉得判断很多。不是 Hermes 喜欢复杂，而是现实世界的 provider 并不干净。
...
这几行很关键。fallback 不是只把 `model` 改了。它至少要同步：
...
这又体现了 api_mode 的意义。Anthropic client 和 OpenAI-compatible client 的重建方式不同，凭证切换不能只改一个字符串。
...
这个设计值得注意，因为它说明 Hermes 的 provider 层有两类扩展：
...
Hermes 的 provider 层可以用一句话概括：`ProviderProfile` 管供应商身份和怪癖，`ProviderTransport` 管 API 协议转换。
