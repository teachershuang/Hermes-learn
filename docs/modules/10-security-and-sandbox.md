...
这一讲要建立一个判断标准：Hermes 哪些地方是真正阻断，哪些地方只是防误操作，哪些地方只是降低模型被外部内容诱导的概率。
...
这个边界值得记住：Hermes 允许模型帮助减少审批噪音，但不让模型决定所有安全底线。
...
还有 `HERMES_WRITE_SAFE_ROOT`。一旦设置，写入只允许落在这些 safe roots 里。这个设计适合服务器部署：你可以把 agent 写权限限制在某个项目目录或数据目录。
...
这很重要。SSRF 风险不只发生在“网页工具”。图片下载、网关媒体缓存、浏览器跳转、skill hub 下载，都可能变成请求内网的入口。
...
源码注释也承认一个限制：DNS rebinding 这种 TOCTOU 问题，预检层解决不了。要彻底解决，需要连接层验证或 egress proxy。这是安全设计里很重要的诚实边界。
...
有一些 backend 可以提供隔离执行环境，比如 Docker、Daytona、Vercel sandbox 等。但本讲看到的很多逻辑不是 OS 级 sandbox，而是 agent runtime guardrail。
