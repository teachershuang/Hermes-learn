...
这和 Claude Code 的 `/compact <focus>` 思路相近：用户告诉压缩器“这次最该保留什么”。比如长会话里既聊了架构、又修了 bug、又讨论部署，可以让压缩重点保留“部署问题”，其它内容使用更高压缩率的摘要。
...
Hermes 的压缩代码横跨 `context_compressor.py`、`conversation_compression.py`、`turn_context.py`、`conversation_loop.py` 和 `hermes_state.py`，因为压缩不是局部文本处理，而是一次会话状态迁移。
