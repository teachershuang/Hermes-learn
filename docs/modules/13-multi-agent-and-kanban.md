...
`delegate_task` 接近函数调用，Kanban 是持久工作队列，worktree 提供并行修改时的文件隔离。三者可以组合，但不能互相替代。
...
源码与公开 issue 中会出现 `delegate_task_async`、`check_task`、`collect_task`、`steer_task` 等名称。公开 issue 将它们描述为非阻塞后台委派的提案，用于让父 Agent 在 child 运行时继续工作。
...
“快速审查三个文件并把结论交给当前对话”适合 `delegate_task`。“研究、实现、review、反复修改并保留人工决策”适合 Kanban task 和依赖图。多个 coding worker 同时修改代码时，每个 worker 应使用独立 worktree，并把验证结果写入 handoff metadata。
