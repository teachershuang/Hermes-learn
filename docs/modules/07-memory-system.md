...
SessionDB、`session_search` 和 Context Compression 都会影响历史信息的可见性，Memory 处理的是另一类生命周期更长的信息。
...
因此，用户在一个 session 中要求 Agent“记住这个偏好”后，内容可以立即写入文件，但不会自动出现在同一轮后续的 system prompt 中。tool result 会说明写入成功；它通常要等到下一次 session 或压缩触发 prompt rebuild 后，才会成为 system prompt 的稳定背景。
...
| session_search | 从 SessionDB 找旧会话片段 | “上次是怎么处理的” |
...
后台 review 的隔离边界是：可以更新长期信息，但不能把 review 自己的消息写进主会话。
...
Hermes 没有把所有“记住的东西”放进一个向量库，因为这些信息的生命周期不同。用户偏好、环境事实、会话原文、当前任务状态和可复用流程分别进入 Memory、SessionDB、live context 或 Skills。
