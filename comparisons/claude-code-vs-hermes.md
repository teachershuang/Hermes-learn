...
Claude Code 和 Hermes 都能阅读代码、调用工具、延续会话、加载项目指令并派生子代理。只看功能表，它们很像。真正读到工程层后，差别很明显：Claude Code 围绕“本地开发会话”组织能力，Hermes 围绕“可长期运行、可接入多个平台的通用 Agent”组织能力。
...
先说明证据边界。`anthropics/claude-code` 是公开仓库，但仓库里的主要内容是插件、示例配置、运维脚本、变更记录和问题跟踪，并不包含 Claude Code 核心运行时的完整源码。Claude Agent SDK 的 Python 部分有源码，不过 SDK 最终会启动 Claude Code CLI。它能证明公开接口怎样调用 CLI，不能证明 CLI 内部怎样实现 Agent loop。
...
Hermes 是开源实现，可以沿调用链读到 Agent loop、工具调度、会话存储、Gateway 和安全检查。这种可见性差异本身就是两者最重要的工程区别之一。
...
先记住这个判断：Claude Code 首先是 coding agent 产品，Hermes 首先是通用 Agent runtime 加多入口产品。两边都在扩张，但源码的主干仍然受这个起点影响。
...
Hermes 的子代理本质上仍是 `AIAgent`，所以可以继承 Provider、上下文引擎、工具系统和插件生命周期。它与 Kanban/worktree 还有另一条面向长期任务的编排路径。
...
如果你在设计 coding agent，Claude Code 的 per-agent 权限和 worktree isolation 很值得参考；如果你在设计跨 Provider、跨平台的通用 Agent，Hermes 的统一 AIAgent 子实例和后台回流机制更容易复用已有运行时。
...
Claude Code 的外部协议已经相当丰富。我们可以准确学习：
